import * as fromOsm from "./fromOpenStreetMap.js";
...
export function mapDetailsToPlaceContext(input) {
  const place = {
    ids: [
      "osm/" +
        fromOsm.osmElementsAbreviations[input.osm_type?.toLowerCase()] +
        "/" +
        input.osm_id,
  ...
  address.text = fromOsm.getAddressText(address);
...
export function basicMapping(osmArea) {
  ...
  const area = {
    ids: [
      "osm/" +
        fromOsm.osmElementsAbreviations[osmArea.osm_type?.toLowerCase()] +
        "/" +
        osmArea.osm_id,
  ...
  const config = fromOsm.countryConfig;

  const mapping = config[countryCode]?.mappings.find(
    (m) =>
      m.filters.nominatim != null &&
      m.filters.nominatim.admin_level == osmArea.admin_level
  );
  ...
  if (mapping != null) {
    ...
    area.ids.push(
      countryCode +
        ...
        mapping.typeOther +
        "/" +
        core.sluggify(osmArea.localname)
    );
  }
  ...
  if (osmArea.type == "postal_code") {
    ...
    area.ids.push(
      countryCode + "/postal_codes/" + core.sluggify(osmArea.localname)
    );
  }
...
export function mapLookupResult(result) {
  ...
  if (isEmpty(result.extratags) == false) {
    ...
    if ("ISO3166-1:alpha3" in result.extratags)
      place.ids.push("places/" + result.extratags["ISO3166-1:alpha3"]);
    if ("ISO3166-1:numeric" in result.extratags)
      place.ids.push("places/" + result.extratags["ISO3166-1:numeric"]);
  }
...
export function mapPhotonResult(result) {
  ...
  if (properties.name != null) place.name = properties.name;
...
export function extractPhotonPlaces(result) {
  ...
  for (const layer of photonLayers) {
    ...
    const idPrefix = bePhotonTemplates.skipOsmPrefix
      ? "places/"
      : "places/osm/";
