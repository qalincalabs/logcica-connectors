...
function propertyRenaming(product) {
  ...
  product.renameProperty("s:image", "images");
...
function assignTranslation(product, propertyName, mainLanguage, languages) {
  let prop = product[propertyName].find((n) => n["@language"] == mainLanguage);

  if (prop == null) prop = product[propertyName][0];

  product[propertyName] = prop["@value"];
}
...
test("OpenBatra clean up", () => {
  ...
  const mainLanguageToExtract = "fr";
  const languagesToExtract = ["fr"];
  ...
  assignTranslationToProperties.forEach((p) => {
    assignTranslation(product, p, mainLanguageToExtract, languagesToExtract);
  });
  ...
  product.images = [{
    url: product.images["s:url"]["@id"]
  }]

  product.ingredients = product.ingredient.map(i => ({
    ingredient: {
        name: i.ingredientName.find(n => n["@language"] == mainLanguageToExtract)["@value"]
    },
